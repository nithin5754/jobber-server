import { findOneBuyer } from '../../Database/Mongoose/Repositories/buyer.repository';
...
export class GetBuyerUsecase {
  public async execute(input: IGetBuyerDTO): Promise<IGetBuyerResult> {
    const found: IRepoResponse = await findOneBuyer({
      buyer: input.email ? { email: input.email } : input.userId ? { _id: input.userId } : { username: input.username }
    });
