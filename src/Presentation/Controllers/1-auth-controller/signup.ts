...
export class SignUp {
  ...
  onCreateUser = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
      let profilePicture: Express.Multer.File | undefined = req.file;

      if (!profilePicture) {
        throw new BadRequestError('file is missing', 'SignUp create() method error');
      }
      ...
      const checkUserExist = await this.authService.usernameOrEmail(email, firstLetterUpperCase(username));
      if (checkUserExist.email) {
        throw new BadRequestError('Error. Email already exist', 'SignUp create() method error');
      } else if (checkUserExist.username) {
        throw new BadRequestError('Error . Username already exist', 'SignUp create() method error');
      } else if (checkUserExist.email && checkUserExist.username) {
        throw new BadRequestError('Error . Username and Email already exit,', 'SignUp create() method error');
      }

      let dataURI: string = this.authService.convertFileToString(profilePicture) as string;
      const profilePublicId: string = this.authService.createUuid();
      const uploadResult: UploadApiResponse = (await this.authService.uploadsPhotos(
        dataURI,
        `${profilePublicId}`,
        true,
        true
      ...
      if (!result) {
        throw new BadRequestError('new user creating  error. Try again', 'SignUp create() method error');
      }
      ...
      if (result && result.username) {
        const messageDetails: IEmailMessageDetails = {
          ...
          receiver: result.email as string,
          locals: {
      ...
      if (!accessToken || !refreshToken) {
        throw new BadRequestError('server error creating new user. Try again', 'SignUp create() method error');
      }
