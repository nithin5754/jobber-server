...
export class VerifyEmail {
  ...
  update = async (req: Request, res: Response, next: NextFunction):Promise<any> => {
    ...
    try {
      ...
      await this.authservice.getUpdateVerifyEmailField((checkIfUserExist.id as string),true)


      let updatedUser:IUser|undefined=await this.authservice.getFetchDataById(checkIfUserExist.id!)
    ...
    return  res.status(StatusCodes.OK).json({ message: 'Email verified successfully.', user: this.userData });
